...
export function HeroHome({ content: c }: { content: HeroHomeContent }) {
  ...
  const headingIsImage = isImageValue(c.heading);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          <div className="lg:col-span-7 space-y-8">
            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-gray-900 leading-relaxed">
              {headingIsImage ? (
                <MaybeImage value={c.heading} imgClassName="max-w-full h-auto" />
              ) : (
                lines.map((line, i) => {
    ...
    <section className="relative bg-gradient-to-br from-gray-50 via-white to-primary-50 py-20 md:py-28 overflow-hidden">
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="heading-xl mb-6 whitespace-pre-line">
              ...
              {c.stats.map((s, i) => (
                <div key={i} className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-white/20">
                  <div className="text-3xl md:text-5xl font-bold font-display mb-2">
                    <MaybeImage value={s.value} />
                  </div>
                  <div className="text-xs md:text-base text-white/80">
                    <MaybeImage value={s.label} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-3xl p-12 text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {c.primaryCta && (
              ...
              >
                <MaybeImage value={c.primaryCta.text} />
              </Link>
            )}
            {c.secondaryCta && (
              <Link
              ...
              >
                <MaybeImage value={c.secondaryCta.text} />
              </Link>
            )}
          </div>
        </div>
...
/* -------------------- image-block -------------------- */
export function ImageBlock({ content: c }: { content: ImageBlockContent }) {
  if (!c.image) {
    return (
      <section className="py-10 bg-white">
        <div className="container-custom">
          <div className="max-w-2xl mx-auto p-8 border-2 border-dashed border-gray-300 rounded-xl text-center text-gray-400 text-sm">
            ยังไม่มีรูปภาพ — แก้ไขเซคชั่นนี้แล้วอัปโหลดรูป
          </div>
        </div>
      </section>
    );
  ...
  const textAlign =
    align === "left"
      ...
      : align === "right"
        ? "text-right"
        : "text-center";
  const rounded = c.rounded ?? true;
  const maxWidth = c.maxWidth || "900px";
    ...
    <section className="py-10 md:py-14 bg-white">
      <div className="container-custom">
        <div className={`flex ${justify}`}>
          ...
          >
            <div
              className={`relative w-full ${rounded ? "rounded-2xl" : ""} overflow-hidden shadow-sm bg-gray-50`}
            >
              <CmsImage
                src={c.image}
                alt={c.alt || ""}
            ...
            {c.caption && (
              <figcaption
                className={`mt-3 text-sm text-gray-500 ${textAlign}`}
              >
                {c.caption}
              </figcaption>
            )}
